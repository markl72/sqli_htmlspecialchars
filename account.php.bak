	...
	try{
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
        			echo " <p>Name: " . htmlspecialchars($row["last_name"]). ", " . htmlspecialchars($row["first_name"]). " <BR>Address: " . htmlspecialchars($row["address"]). "<br>Phone number: " . htmlspecialchars($row["phone_no"]) . "</p>";
    			}
		}
		else
		{
